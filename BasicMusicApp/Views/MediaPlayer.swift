...
final class MediaPlayer: UIView {
  ...
  init(album: Album, playingIndex: Int) {
    self.album = album
  ...
  private func setupView() {
    ...
    albumCover.image = UIImage(named: "no_image")
    setupPlayer(song: album.songs[self.playingIndex])
    [albumName, songNameLabel, artistNameLabel, elapsedTimeLabel, remainingTimeLabel].forEach { (v) in
      v.textColor = .white
    }
  ...
  private func setupConstraints() {
    ...
    NSLayoutConstraint.activate([
      ...
      albumCover.topAnchor.constraint(equalTo: albumName.bottomAnchor, constant: 24),
      albumCover.heightAnchor.constraint(equalToConstant: UIScreen.main.bounds.height * 0.45)
    ])
  ...
  private func setupPlayer(song: Song) {
    print("resource path = ", song.fileURL)

    if timer == nil {
      timer = Timer.scheduledTimer(timeInterval: 0.00001, target: self, selector: #selector(updateProgress), userInfo: nil, repeats: true)
    }
  ...
  func play() {
    ...
    progressBar.maximumValue = Float(self.player?.duration ?? 0.0)
    self.player!.play()
    setPlayPauseIcon(isPlaying: self.player!.isPlaying)
  }
...
//    timer = nil
  ...
  @objc private func updateProgress() {
    progressBar.value = Float(player?.currentTime ?? 0.0)

    elapsedTimeLabel.text = getFormattedTime(timeInterval: player?.currentTime ?? 0.0)
    let remainingTime = (player?.duration ?? 0.0) - (player?.currentTime ?? 0.0)
  ...
  @objc private func didTapNext(_ sender: UIButton) {
    playingIndex += 1
    print("album songs count", album.songs.count)
  ...
  private func getFormattedTime(timeInterval: TimeInterval) -> String {
    ...
    guard let minsString = timeFormatter.string(from: NSNumber(value: mins)),
          let secString = timeFormatter.string(from: NSNumber(value: secs)) else {
      return "00:00"
    }
